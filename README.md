...
- 使用地区与来源筛选器联动更新观点和证据。
- 向 AI 游戏顾问提问，实时查看 GLM 回答流，并通过证据编号和来源 URL 回看原始公开页面。
...
`npm run dev` 会启动本地渲染服务并自动打开 **ReHoYo Electron 桌面窗口**。Vite 不会打开浏览器，产品也会拒绝新窗口与外部导航。未配置 GLM 时应用先显示全屏安全连接页，不会进入任务大厅，也不提供离线模拟回退。
...
报告完成后，顾问通过 GLM `chat/completions` 的真实 Server-Sent Events 响应逐片显示答案。主进程发送 `stream: true`，增量解析 `choices[0].delta.content`，再通过带请求 ID 的窄 IPC 事件把文本分片交给渲染层；这不是前端打字机动画，也不会伪造等待过程。

渲染层把已收到的完整 Markdown 快照交给 Streamdown 2.5，因此生成中的标题、列表、引用、表格、链接和代码块能够在语法尚未闭合时稳定显示。原始 HTML 被禁用，Markdown 链接只允许 HTTPS 或当前页面锚点。模型返回的正文不会决定引用关系：右侧证据链与引用按钮仍由提问前匹配到的真实 `EvidenceRecord` 生成。

- 生成期间“发送问题”切换为“停止生成”；停止会中止主进程网络请求并保留已收到的内容。
- 首个真实分片到达前失败时，界面明确回退到本地证据结论。
- 已收到分片后失败时，界面保留部分回答并标记“回答未完成”，不会用另一段内容覆盖。
- 页面关闭或顾问工作区卸载时，未完成请求自动取消并清理监听器。

设计与实施计划见 [`docs/superpowers/specs/2026-07-23-streaming-advisor-markdown-design.md`](./docs/superpowers/specs/2026-07-23-streaming-advisor-markdown-design.md) 与 [`docs/superpowers/plans/2026-07-23-streaming-advisor-markdown.md`](./docs/superpowers/plans/2026-07-23-streaming-advisor-markdown.md)。接口格式依据[智谱流式消息文档](https://docs.bigmodel.cn/cn/guide/capabilities/streaming)，Markdown 行为依据 [Streamdown 文档](https://streamdown.ai/docs/usage)。

### 配置 GLM 连接

ReHoYo 支持两种连接方式，配置优先级为 **显式外部文件 / 环境变量 > 应用内加密配置**。两种方式都不允许把 API Key 写入项目文件、Git、`localStorage` 或日志；模型端点固定为 `https://open.bigmodel.cn/api/coding/paas/v4`，渲染器不能覆盖。
...
- 预加载层只暴露只读运行状态、受限研究任务与带请求 ID 的顾问流式/取消/事件桥接，不暴露密钥或文件路径。
...
├── glm-client.mjs        GLM 配置校验、请求净化与 SSE 流式客户端
