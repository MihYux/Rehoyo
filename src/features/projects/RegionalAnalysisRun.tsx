import { ArrowLeft, CircleNotch, Database, GlobeHemisphereWest, WarningCircle } from '@phosphor-icons/react'
...
export function RegionalAnalysisRun({ project, onComplete }: Props) {
  ...
  return <div className="regional-run-page">
    ...
    <main className="regional-run-main">
      <section className="regional-run-heading"><span>AGENT RESEARCH</span><h1>Agent正在理解不同区域。</h1><p>每条到达的观点都必须带HTTPS来源和原始摘录。没有命中就显示0，不补造评论。</p><div className="regional-run-status">{running ? <><CircleNotch className="spin" size={20} /> {latest?.message || '正在建立区域检索队列…'}</> : error ? <><WarningCircle size={20} /> {error}</> : <><Database size={20} /> 真实证据已交接</>}</div><div className="regional-run-coverage"><div><span>站点目标</span><strong>{latestCoverage?.sitesAttempted ?? 0} / 30+</strong></div><div><span>真实证据</span><strong>{evidence.length} / 30+</strong></div><div><span>检索组合</span><strong>{providers.length ? providers.map((item) => item === 'brave' ? 'Brave' : 'BigModel').join(' + ') : '正在启动'}</strong></div></div></section>
