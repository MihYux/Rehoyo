import { ArrowLeft, ArrowRight, Check, Flag, Package, Sparkle } from '@phosphor-icons/react'
...
export function ReleaseProjectForm({ onCreate }: Props) {
  ...
  const [objective, setObjective] = useState<Objective>('activity')
  const [pointType, setPointType] = useState<SellingPoint['type']>('character')
  ...
  function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const pointName = String(data.get('sellingPointName') || '').trim()
      const brief: VersionReleaseBrief = {
        ...
        sellingPoints: [{
          ...
          type: pointType,
          name: pointName,
          description: String(data.get('sellingPointDescription') || '').trim(),
          priority: 'primary',
        ...
        budgetLevel: String(data.get('budgetLevel') || 'medium') as VersionReleaseBrief['budgetLevel'],
        teamCapacity: ['发行策划', '社媒运营'],
        mandatoryActions: assets.includes('版本PV') ? ['版本PV'] : [],
        prohibitedActions: ['未经审核的自由角色聊天', '未经验证的商业效果承诺'],
        riskPreference: String(data.get('riskPreference') || 'balanced') as VersionReleaseBrief['riskPreference'],
        allowCharacterRelationshipPilot: allowCharacter,
      }
      const project = createReleaseProject({
        game: chosenGame,
        version: String(data.get('version') || ''),
        updateName: String(data.get('updateName') || ''),
        releaseAt: `${String(data.get('releaseAt') || '')}T00:00:00.000Z`,
        cycleDays: 42,
        regions,
        brief,
      })
      ...
      <form className="release-brief-form" onSubmit={submit}>
        <section className="brief-chapter" id="brief-01" aria-labelledby="brief-title-01">
          ...
          <div className="brief-chapter__body">
            <fieldset className="brief-game-options"><legend>游戏</legend>
              {games.map((item) => <button type="button" key={item.name} className={!showCustom && game === item.name ? 'is-selected' : ''} aria-label={`选择游戏 ${item.name}`} onClick={() => { setGame(item.name); setShowCustom(false) }}><span>{item.code}</span><strong>{item.name}</strong>{!showCustom && game === item.name && <Check size={16} />}</button>)}
            </fieldset>
            <button className="brief-text-link" type="button" onClick={() => setShowCustom((current) => !current)}>{showCustom ? '使用预设游戏' : '输入其他游戏'}</button>
            ...
            <div className="brief-field-row">
              <label className="brief-field"><span>版本号</span><input name="version" aria-label="版本号" required placeholder="例如 3.8" /></label>
              <label className="brief-field"><span>预计上线日期</span><input name="releaseAt" aria-label="预计上线日期" type="date" required /></label>
            </div>
            <label className="brief-field"><span>更新名称</span><input name="updateName" aria-label="更新名称" required placeholder="用团队内部确认的版本名称" /></label>
          </div>
        </section>
        ...
        <section className="brief-chapter" id="brief-03" aria-labelledby="brief-title-03">
          ...
          <div className="brief-chapter__body">
            ...
            <label className="brief-field"><span>核心卖点名称</span><input name="sellingPointName" aria-label="核心卖点名称" required placeholder="例如：三月七全新形态" /></label>
            <label className="brief-field"><span>核心卖点说明</span><textarea name="sellingPointDescription" aria-label="核心卖点说明" required rows={4} placeholder="只写已确认的版本内容与边界" /></label>
          </div>
        </section>
        ...
        <section className="brief-chapter" id="brief-04" aria-labelledby="brief-title-04">
          ...
          <div className="brief-chapter__body">
            ...
            <div className="brief-field-row">
              <label className="brief-field"><span>预算档位</span><select name="budgetLevel" defaultValue="medium"><option value="low">低</option><option value="medium">中</option><option value="high">高</option></select></label>
              <label className="brief-field"><span>风险偏好</span><select name="riskPreference" defaultValue="balanced"><option value="conservative">保守</option><option value="balanced">平衡</option><option value="experimental">实验</option></select></label>
            </div>
